...
public class ReceiveAdapter extends RecyclerView.Adapter<ReceiveAdapter.MyViewHolder> {
    ...
    private ReceivedRicesFragment fragment;
    private static List<Tahvil> list;
    private Farmer farmer;

    public ReceiveAdapter(ReceivedRicesFragment fragment, List<Tahvil> list) {
        this.fragment = fragment;
        ReceiveAdapter.list = list;
    }
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        Tahvil tahvil = list.get(position);

        Application.getShaliAPI().searchFarmer(String.valueOf(tahvil.getBerenj())).enqueue(new Callback<List<Farmer>>() {
            @Override
        ...
        String date_fixed = fix_date(tahvil.getTarikh());

        holder.code.setText("کد " + tahvil.getId());
        holder.date.setText(date_fixed);
        holder.time.setText(tahvil.getSaat());
        holder.majmoe_vazn_berenj.setVisibility(View.GONE);
        holder.tedad_kise_berenj.setText("تعداد کیسه برنج: " + tahvil.getTedad_kise_berenj());
        holder.vazn_kise_berenj.setVisibility(View.GONE);
        holder.nimdane_weight.setText("وزن نیم دانه: " + tahvil.getVazn_nimdane() + " کیلوگرم");
        holder.tedad_kise_saboos.setText("تعداد کیسه سبوس: " + tahvil.getTedad_kise_saboos());
